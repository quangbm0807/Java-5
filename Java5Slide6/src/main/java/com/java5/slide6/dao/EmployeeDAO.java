...
public interface EmployeeDAO extends JpaRepository<Employee, Long>  {
	...
	//Report
	@Query("SELECT new com.java5.slide6.entity.DepartmentEmployeeSalaryReport(e.department, sum(e.salary)) "
			+ "	from Employee"
			+ "	 e group by e.department ORDER BY e.department")
	List<DepartmentEmployeeSalaryReport> getTotalSalaryDepartment();
	
	//Query DSL (Domain Specific Language)
	List<Employee> findBySalaryBetweenOrderBySalary(Double min, Double max);
